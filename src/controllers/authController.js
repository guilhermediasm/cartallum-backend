...
router.post('/authenticate', async (req, res) => {
  ...
  try {
    if (await Instituicao.findOne({ email })) {
      ...
      instituicao = await Instituicao.findOne({ nomeInstituicao }).select(
        '+password',
      );
    } else {
      return res.status(200).send({
        success: false,
        msg: 'Essa instituição não exite, favor cadastrar',
      });
    }
    ...
    if (!(await bcrypt.compare(password, instituicao.password))) {
      return res
        .status(200)
        .send({ success: false, msg: 'A senha e inválida' });
    }
    ...
    // TODO shouldn't it return 401 - unauthorized?
    return res.status(200).send({
      success: false,
      msg: 'Ocorreu um erro na autenticação, por favor tente mais tarde',
      erro: err,
    });
  }
});
